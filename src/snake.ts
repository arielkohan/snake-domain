...
export class Snake {
  ...
  passingThroughCoordinates(coordinates: Coordinates): boolean {
    return this.snakeCoordinates.some(
  ...
  addDirectionCommand(newDirection: Directions) {
    ...
    if (validDirection) this.pendingDirectionCommands.push(newDirection);
  }
  ...
  private newDirectionValid(newDirection: Directions) {
    ...
    if (this.pendingDirectionCommands.length) {
      previousDirection = this.pendingDirectionCommands[
        this.pendingDirectionCommands.length - 1
      ];
    }
  ...
  move(foodCoordinates: Coordinates): boolean {
    this.headCurrentDirection =
      this.pendingDirectionCommands.shift() || this.headCurrentDirection;
    const newHeadCoordinates: Coordinates = this.calculateHead(
      this.headCurrentDirection
    );
    this.snakeCoordinates.unshift(newHeadCoordinates);

    const eating =
      newHeadCoordinates.x === foodCoordinates.x &&
      newHeadCoordinates.y === foodCoordinates.y;
